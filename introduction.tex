...
Existing RAG systems, as depicted in \Cref{fig:maestroRAG}, place both the encoding stage (converts queries to embeddings) and the generation stage (produces responses via an LLM) on the GPU~\cite{jin2024flashrag, jiang2024piperag}. On edge devices with single GPU, this creates a \emph{structural hazard}: encoding and generation cannot run concurrently, leading to pipeline bubbles and resource underutilization. Prior systems like FlashRAG~\cite{jin2024flashrag} and PipeRAG~\cite{jiang2024piperag} target datacenter deployments with abundant GPUs, while EdgeRAG~\cite{EdgeRAG} addresses edge scenarios but neither systematically optimizes CPU--GPU coordination nor performs smart orchestration.
...
\begin{figure*}
    ...
    \caption{\new{Stage placement in a classical RAG pipeline and in \design{}. The classical pipeline places both encoding and generation on the GPU; \design{} places encoding and retrieval on dedicated CPU cores and reserves the GPU for generation.}\flag{Figure text size: this figure's labels measure 4.22\,pt on the page and \Cref{Fig:OurPipelines} measures 4.40\,pt, both below the 8\,pt the shepherd asked for. Every data figure was regenerated and clears 8\,pt, but these two are exported artwork with no editable source in the repository, so they could not be regenerated. Supply the source files or replacement exports before submitting. Panel (c) of \Cref{Fig:OurPipelines} also reads ``FlashFAG'' for ``FlashRAG'', which needs the same source file to fix. The white-on-light-green issue is already resolved: this figure's labels are black.}}
    \label{fig:maestroRAG}
\end{figure*}

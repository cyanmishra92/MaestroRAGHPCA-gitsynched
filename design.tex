...
Separating out augment and retrieval stages, although brings cache hit benefits in their respective private caches, but suffers from synchronization and data transfer overheads across processes (resulting in latency increase of the two stages --  up to 128 ms on average)\flag{This ``--'' is a parenthetical inside a parenthesis. A colon or a comma would read better, and the style rule for this revision bans dashes used this way. Not changed here because Part E is audit-only.}, overshadowing the benefits from cache hits.

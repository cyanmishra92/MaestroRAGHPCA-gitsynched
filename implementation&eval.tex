...
\flag{Three appositive ``--'' dashes in this sentence, after each of the three baseline citations. The Task 8 style rule bans dashes used this way; the C2 sweep listed only the --- and U+2014 forms, so these survived. A colon or a comma would fix each. Not changed here because Part E is audit-only.}
...
The power and energy measurement results are reported in \Cref{tab:power_energy}. MaestroRAG significantly outperforms FlashRAG and PipeRAG in energy efficiency, only consuming $253.3J/query$, compared to \new{$792.00J$} and $780.92J/query$ for FlashRAG and PipeRAG, respectively. Moreover, FlashRAG and PipeRAG exhibit \new{$16.36\%$} and \new{$8.08\%$} higher peak CPU power usage, respectively.
...
\flag{\Cref{tab:latency_breakdown} fuses \flashRAG{}'s encode and retrieve, while \Cref{tab:power_energy} fuses its retrieve and augment and reports no encode share at all. Both groupings are the rebuttal's own, but a reader comparing the two tables will see \flashRAG{} categorised two ways. Confirm the spanned cells and the n/r marker make that legible, or add one sentence saying the groupings differ between the latency and energy measurements.}
...
\flag{This Jetson caching claim has no support: \Cref{tab:cache_results} has no Jetson column and the workbook holds no caching data at all. Delete it, requalify it as an expectation, or measure it.}

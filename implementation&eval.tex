...
\noindent\textbf{Metrics:} We report two key performance metrics for the end-to-end pipeline. The first is "latency", which is the total completion time for a batch of queries. The second is "throughput" (qps), defined as the number of queries processed per second. In the results, we compare our proposed \design{} against three baseline approaches: (1) FlashRAG~\cite{jin2024flashrag}, a modular RAG framework where both encoding and generation are placed on the GPU; (2) PipeRAG~\cite{jiang2024piperag}, a server-oriented pipeline design; and (3) EdgeRAG~\cite{EdgeRAG}, which mitigates memory usage by caching only coarse indices and generating finer embeddings on demand using the GPU.
...
In contrast, dedicating only generation to the GPU keeps our pipeline stable for larger batches. 
...
\subsection{\new{Latency on the Embedded Platform}}
\label{subsec:evalJetson} %new
...
we eliminate large data copies and cut overhead relative to EdgeRAG by \new{up to $26\%$}. Although gains are smaller than on larger GPUs, they illustrate that our pipeline and data-sharing optimizations still yield meaningful benefits on edge devices.
\begin{figure}[t]
  \centering
  % CR-REPLACED: \includegraphics[width=0.9\linewidth]{Figs/JetsonThemVsUs.pdf}
  \includegraphics[width=0.65\linewidth]{Figs/cr/JetsonThemVsUs.pdf}
  \caption{Latency comparison of EdgeRAG and \design{} on a Jetson~AGX~Orin with 1\,M database.}
  \label{fig:JetsonThemVsUs}
\end{figure}

% CR-REPLACED: \begin{figure*}[t]
% CR-REPLACED:     \centering
% CR-REPLACED: % CR-REPLACED: \includegraphics[width=0.8\linewidth]{Figs/power_energy_comparison.pdf}
% CR-REPLACED:     \includegraphics[width=\linewidth]{Figs/cr/fig6_power_energy_breakdown.pdf}
...
% CR-REPLACED:     \label{fig:powerEnergyPlot}
% CR-REPLACED:     \vspace{-10pt}
% CR-REPLACED: \end{figure*}
\input{TablesAlgos/PowerEnergy}
...
On the RTX\,4090, our method completes inference in 6.50\,s, which is \new{$2.5\times$ and $3.0\times$} faster than \flashRAG{} (16.39\,s) \new{and} \pipeRAG{} (19.80\,s)\new{, respectively}, and \new{$4.4\times$} faster than \edgeRAG{}\new{, whose stage-level composition is given in \Cref{sec:breakdown}}.
On the RTX\,4080, our pipeline finishes in 3.96\,s, outperforming \edgeRAG{}, \flashRAG{}, and \pipeRAG{} by up to 8.8$\times$, \new{$2.0\times$}, and 5.4$\times$, respectively. 
...
We also measure tail latency and amplification (\textit{P99}, \textit{P99/P50}) at \texttt{BS=8}, \texttt{DB=2\,M}. Our system achieves 5.26\,s (1.04), outperforming FlashRAG (14.86\,s, 1.14) and PipeRAG (14.41\,s, 1.09).\flag{Untraced: these six tail-latency figures appear in no authorised workbook tab and in no rebuttal sentence. They are internally consistent, since each P99 divided by its ratio reproduces the BS=8, DB=2\,M column, but the measurements themselves have no recorded source. Supply the data or drop the sentence.}
...
To assess throughput, we simulate bursty traffic from scaled Azure traces~\cite{azureTrace}, processing 256 queries on the RTX\,4090 and 128 in Jetson, injected every 10\,s and 15\,s, respectively. As shown in \Cref{Fig:ThroughputResults}, we set \texttt{DB=4\,M} and \texttt{BS=8}, consistent with the mid-scale latency experiments. On the RTX\,4090, \design{} reaches 1.60\,QPS, ahead of \edgeRAG{} at \new{0.288}\,QPS, \flashRAG{} at \new{0.683}\,QPS, and \pipeRAG{} at \new{1.185}\,QPS. Although the latter two are more GPU-conscious, they still incur frequent data movements and memory overheads that collectively restrict concurrency under large-batch bursts. In contrast, our CPU-based encoding and retrieval allow asynchronous, multi-query execution without overburdening GPU memory. In Jetson, \design{} achieves \new{0.432}\,QPS, which is 6.7$\times$ better than \edgeRAG{} (\new{0.0645}\,QPS) and about 1.16$\times$ faster than \pipeRAG{} (\new{0.370}\,QPS). \flashRAG{} relies on vLLM~\cite{vllm}, which is not supported on Jetson, so it is omitted. Despite the overall lower speed on the Jetson, decoupling retrieval and encoding on the CPU increases concurrency under limited DRAM and sustains higher steady-state throughput.
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
 \begin{figure*} [ht!]
    ...
    {%
     % CR-REPLACED: \includegraphics[clip,width=0.32\linewidth]{Figs/cores_allocation_stacked.pdf}%
    ...
    {%
    % CR-REPLACED: \includegraphics[clip,width=0.32\linewidth]{Figs/MainLatencyResults2.pdf}%
    ...
    {%
    % CR-REPLACED: \includegraphics[clip,width=0.32\linewidth] {Figs/ThroughputResults.pdf}%
    \includegraphics[clip,width=0.32\linewidth]{Figs/cr/ThroughputResults.pdf}%
    \label{Fig:ThroughputResults}
    }
    \caption{\new {(a) Adaptive Worker (\#Cores) Allocation using \design{} allocator against an empirical one (b) End-to-end latency comparison for configurations selected by the adaptive worker allocator (c) Throughput comparison for configurations selected by the adaptive worker allocator. Panel (b) uses \texttt{BS=8} with 4\,M, 4\,M and 1\,M databases on the RTX\,4090, RTX\,4080 and Jetson respectively, and panel (c) uses \texttt{DB=4\,M} and \texttt{BS=8}. N/C marks \flashRAG{} on Jetson, which relies on vLLM and is therefore not compatible with that platform. 
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\input{TablesAlgos/CachingTable}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
Overall, the cache policy provides a tunable, memory-efficient approach that leverages session-level query locality.
...
\noindent\textbf{Adaptive Batching and Worker Allocation: }In \Cref{Fig:cores_allocation_stacked}, we compare the recommended batch-size and core-allocation between our adaptive mapper (\Cref{subsec:resource-mapping}) and exhaustive grid searching. The bars labeled ``Mapper~Identified'' show the mapper’s suggested allocations, while the ``Empirically~Observed'' bars come from direct measurement. The results on the RTX\,4090 for four database sizes (1\,M, 2\,M, 4\,M, 8\,M) indicate that the mapper usually matches or outperforms purely empirical approaches, particularly at 2\,M and 4\,M. For instance, with a 4\,M database, the mapper selects \texttt{BS=8} and assigns 2~cores to encoding and 6~cores to retrieval, which indeed yields strong measured performance. Occasionally the mapper opts for fewer encoding cores to keep the retrieval scaling robust. Exhaustive grid searches quickly become infeasible on real systems, especially as the database or query volume increases. In contrast, the mapper adapts well to new hardware or changing data sizes, making it especially useful in constrained environments that cannot afford extensive trial configurations.
...
\noindent\textbf{Impact of indexing schemes: }In addition to the flat indexing scheme for accessing the database, we also experimented with three other indexing schemes, namely approximate nearest neighbor (ANN) methods~\cite{l2,faiss}, IVF-Flat, IVF-PQ~\cite{ivfpq}, and HNSW~\cite{hnsw}, to understand the performance implications. All alternatives provided substantial latency improvements, with observed gains of $29.27\%$, $28.15\%$, and $32.18\%$ respectively. The choice of index therefore trades retrieval accuracy against latency.

\noindent\textbf{Impact of optimization mechanisms: }To isolate the impact of individual optimizations, we performed an ablation study of end-to-end latency. Starting from a baseline latency of $15.12s$ (no optimizations), we observe incremental improvements: adaptive batching and core allocation reduce the latency to $11.79s$; software-level optimizations bring it down further to $6.497s$; and caching achieves $2.003s$ \new{under the $80\%$ hit rate described in \Cref{sec:software-caching}}.
...
\noindent\textbf{Impact of cold start: } Finally, we evaluate cold-start overhead which consists of latency incurred in model loading, process creation by scheduler for different stages, index loading and other library loads. MaestroRAG incurs a one-time cold-start cost of \new{approximately} $7s$\new{, of which} $0.36s$ \new{is} encoder loading, $3.04s$ \new{vector index construction, and} $3.36s$ \new{loading the generation model onto the GPU; the remainder is process creation and library initialization}. In contrast, FlashRAG and EdgeRAG re-incur this overhead at the beginning of every batch, highlighting a key advantage of our initialization strategy in batch inference settings.

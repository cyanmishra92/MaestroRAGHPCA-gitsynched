...
% Refer to sections as "\S N" and nothing else.
% \creflabelformat only sets the *label* part, so cleveref still prepended its name and
% the result read "Section \S2.1". \crefformat sets the complete reference text, so the
% name is not emitted at all and only the section sign remains.
...
% Same treatment for grouped and ranged references, so the duplication cannot come back
% through a form the paper does not happen to use today.
...
%%%%% REVISION MODE %%%%%
% 0 = camera-ready (all text black)
% 1 = shepherd review (additions in blue)
% 2 = drafting (additions in blue + margin markers)
\newcommand{\revmode}{1}
...
\else
  ...
  \newenvironment{newtext}{\begingroup\color{blue}}{\endgroup}
\fi

%%%%% AUTHOR ATTENTION FLAGS %%%%%
% 1 = show flags in red   0 = hide entirely (camera-ready)
% Independent of \revmode. A flag is a note to the author and never carries text the
% paper needs; every flag must be deletable without leaving a gap in the prose.
...
\title{Regular-MaestroRAG: Orchestrated Pipeline Architecture for Efficient RAG on Edge Devices}
